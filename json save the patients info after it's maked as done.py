...
# List that holds all served patient records
served_patients = []
...
# File where active patient records are saved between sessions
DATA_FILE = "patients.json"

# File where served patient records are permanently saved
HISTORY_FILE = "served_history.json"
...
def save_patients():
    """
    Save the current active patients list to patients.json.
...
def save_served_history():
    """
    Append the current served_patients list to served_history.json.

    Each served patient record includes:
        - All original fields (number, name, id, contact, severity, time)
        - A new "served_time" field: when they were marked as done

    The file grows over time — one record per patient served — so the
    hospital always has a permanent log of every completed visit.
    """
    serialisable = []
    for p in served_patients:
        record = {
            ...
            "time":        p["time"].isoformat(),
            # When the patient was marked as served (new field)
            "served_time": p["served_time"],
        }
        serialisable.append(record)
...
def load_served_history():
    """
    Load previously served patient records from served_history.json on startup.

    If the file does not exist yet (first run) or is corrupted,
    the function silently does nothing — served_patients stays empty.

    The ISO-format time strings are converted back into strings
    since we only display them and don't need datetime arithmetic.
    """
    global served_patients
    ...
    try:
        ...
        loaded = []
        for record in data:
            # Convert check-in time back to datetime for consistency
            record["time"] = datetime.fromisoformat(record["time"])
            loaded.append(record)

        served_patients = loaded
...
def auto_refresh():
    ...
    if "history_listbox" in widgets and widgets["history_listbox"].winfo_exists():
        refresh_history_list(widgets["history_listbox"])
...
def show_staff_dashboard(username):
    ...
    Buttons: Call Next Patient | Mark as Served | View History | Refresh
    ...
    tk.Button(
        btn_frame, text="View History",
        font=("Arial", 10, "bold"),
        bg="#8B4513", fg="white",
        width=14, cursor="hand2",
        command=lambda: show_served_history(username)
...
def serve_next_patient(dash_listbox, count_label):
    """
    Remove the highest-priority patient from the active queue,
    record them in served_patients with a served_time timestamp,
    ...
    # ── NEW: stamp the time they were served and move to history ──
    p["served_time"] = datetime.now().strftime("%Y-%m-%d %I:%M:%S %p")
    served_patients.append(p)
    ...
    messagebox.showinfo(
        ...
        f"Record saved to history."
    )

    refresh_dashboard_list(dash_listbox, count_label)


# ─── SCREEN 5: SERVED HISTORY ─────────────────────────────────────────────────

def show_served_history(username):
    """
    Display all patients who have been marked as served.

    Columns: # | Name | ID | Contact | Severity | Check-in | Served At
    Staff can return to the dashboard via the Back button.

    Parameters:
        username (str): Passed through so Back can return to dashboard.
    """
    clear_screen()

    # Top bar
    top = tk.Frame(root, bg="#5a3010")
    top.pack(fill="x")

    tk.Label(top, text="Served Patient History",
             font=("Arial", 13, "bold"),
             bg="#5a3010", fg="white").pack(side="left", padx=12, pady=8)

    tk.Button(top, text="<- Back to Dashboard",
              command=lambda: show_staff_dashboard(username),
              bg="#5a3010", fg="white",
              relief="flat", cursor="hand2").pack(side="right", padx=12)

    clock = tk.Label(top, font=("Arial", 10), bg="#5a3010", fg="white")
    clock.pack(side="right", padx=12)
    widgets["clock_label"] = clock

    # Summary label
    history_count = tk.Label(root, font=("Arial", 9), fg="gray")
    history_count.pack(pady=(6, 0))

    # Column headers
    header = tk.Frame(root, bg="#5a3010")
    header.pack(fill="x", padx=12)

    for col_text, col_width in [
        ("#", 5), ("Name", 14), ("ID", 9),
        ("Contact", 12), ("Severity", 10), ("Check-in", 12), ("Served At", 12)
    ]:
        tk.Label(
            header,
            text=col_text,
    ...
    # Scrollable history list with scrollbar
    list_frame = tk.Frame(root)
    list_frame.pack(padx=12, pady=4, fill="both", expand=True)

    scrollbar = tk.Scrollbar(list_frame)
    scrollbar.pack(side="right", fill="y")

    history_listbox = tk.Listbox(
        list_frame, width=82, height=10,
        font=("Courier", 9),
    ...
    widgets["history_listbox"] = history_listbox

    refresh_history_list(history_listbox)

    # Update summary count
    total = len(served_patients)
    history_count.config(
        text=f"{total} patient(s) served in total"
    )


def refresh_history_list(history_listbox):
    """
    Clear and redraw the served history listbox.
    Shows most recently served patients first (newest at top).

    Columns: # | Name | ID | Contact | Severity | Check-in | Served At

    Parameters:
        history_listbox (tk.Listbox): The listbox widget to update.
    """
    history_listbox.delete(0, tk.END)

    if not served_patients:
        history_listbox.insert(tk.END, "  No patients have been served yet.")
        return

    # Show newest served first
    for p in reversed(served_patients):
        checkin_str = p["time"].strftime("%m/%d %I:%M%p") if isinstance(p["time"], datetime) else str(p["time"])
        served_str  = p.get("served_time", "N/A")

        # Shorten served_time for display (remove date, keep time)
        if len(served_str) > 10:
            served_str = served_str[11:22]  # "HH:MM:SS AM/PM"

        num_col     = "#" + str(p["number"]).zfill(3)
        name_col    = p["name"][:12].ljust(14)
        id_col      = p["id"][:7].ljust(9)
        contact_col = p["contact"][:10].ljust(12)
        sev_col     = p["severity"].ljust(10)
        checkin_col = checkin_str.ljust(12)
        served_col  = served_str

        line = ("  " + num_col + "  " + name_col + "  " + id_col +
                "  " + contact_col + "  " + sev_col + "  " +
                checkin_col + "  " + served_col)
...
if __name__ == "__main__":
    ...
    # Load any previously served patient records
    load_served_history()
